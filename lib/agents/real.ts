...
// ── Connector-as-Agent / Demo Exclusion ──────────────────────────────────────
// These are connectors or demo placeholders, not business agents.
// Business logic agents use these as tools, not as peers.
const EXCLUDED_AGENT_IDS = new Set([
  // Connectors (should be tools, not agents)
  'gmail-worker',
  'whatsapp-worker',
  'slack-worker',
  'stripe-sales',
  ...
  'stack-monitor',
  // Demo / Placeholder agents
  'social-agent',  // merged into social-content-studio
  'sales-agent',   // demo client workflow
  'sales-calls-data',  // demo
  'markdown-auditor',  // internal tool, not a business agent
  'vector-auditor',    // internal tool
  'payments-pulse',    // connector
  'client-roster',     // demo client
  'client-onboarding', // demo client
  'client-success',    // demo client
]);

// Production-filtered agent roster
export const productionAgents: RuntimeAgent[] = realAgents.filter(
  (agent) => !EXCLUDED_AGENT_IDS.has(agent.id)
);
